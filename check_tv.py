...
def find_gmail_title(title):
    ...
    # Convert the times to the GMT format required by the Gmail API
    query = f'after:{minutes_ago.strftime("%Y/%m/%d %H:%M:%S")} before:{now.strftime("%Y/%m/%d %H:%M:%S")} subject:"{title}"'
...
def offline_check(driver, live_url, spare_link, important, titleforgmail):
      ...
      while True:
         ...
         gmailcount += 1
         if gmailcount == 11:
            find_gmail_title(titleforgmail)
         if refresh_count == 60:
            driver.refresh()
            time.sleep(7)
            actions = ActionChains(driver)
...
def get_gmail_service():
  try:
    ...
    if 'userRequestsExceedRateLimit' in error_details[0]['reason']:
        ...
        # Implement a retry mechanism with exponential backoff or other strategy
        # to handle the rate limiting issue
        exit()
    else:
        # Handle other types of HttpError exceptions
